...
pub async fn run_doctor(db_path: &Path, engine: Arc<dyn MemoryEngine>) -> Result<()> {
    ...
    match engine.search("test", Some(&Scope::Global), 1).await {
        Ok(results) => {
            println!(
                "   ✓ Engine read query: OK (returned {} records)",
                results.len()
            );
        }
        Err(e) => {
    ...
    match engine.get_known_failures(None, None, 5).await {
        Ok(failures) => {
            println!(
                "   ✓ Failure pattern repository: OK (active anti-patterns: {})",
                failures.len()
            );
        }
        Err(e) => {
    ...
    check_host_integration(
        "Cursor",
        Path::new(".cursor/rules/strata.mdc"),
...
fn check_host_integration(host_name: &str, primary_file: &Path, secondary_file: &Path) {
    ...
    if p_exists || s_exists {
        println!(
            "   ✓ {host_name}: Configured (found {})",
            if p_exists {
                primary_file.display().to_string()
            } else {
                secondary_file.display().to_string()
            }
        );
    } else {
